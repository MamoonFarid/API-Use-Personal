...
with tab2:
    ...
    with col_b:
        min_price = st.number_input("Min Buy Box Price (£)", value=22.0, step=1.0)
        max_price = st.number_input("Max Buy Box Price (£)", value=55.0, step=1.0)
    with col_c:
        max_bsr = st.number_input("Max Sales Rank (BSR)", value=20000, step=1000)
        max_results = st.slider("Results to Fetch", min_value=10, max_value=100, value=30)
    ...
    if st.button("🔍 Search Keepa Database"):
        query_payload = {
            ...
            "current_AMAZON_lte": -1,  # Amazon Out of Stock
            "current_COUNT_LIVE_OFFERS_SHIPPING_gte": 3,
            "current_COUNT_LIVE_OFFERS_SHIPPING_lte": 8,
        ...
        with st.spinner("Searching Amazon UK database..."):
            url = f"https://api.keepa.com/query?key={api_key}&domain={DOMAIN_UK}&selection={json.dumps(query_payload)}"
            resp = requests.get(url)
            
            if resp.status_code == 200:
                asins = resp.json().get("asinList", [])
                st.write(f"Matched **{len(asins)}** high-demand ASINs.")
                
                if asins:
                    products = api.query(asins, domain=DOMAIN_UK, stats=180, history=False)
                    shortlist = []
                    for p in products:
                        if not p or 'stats' not in p or not p['stats']:
                            continue
